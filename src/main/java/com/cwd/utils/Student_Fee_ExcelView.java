...
public class Student_Fee_ExcelView extends AbstractXlsxView {
	...
	protected void buildExcelDocument(
		...
		List<Student_Fee> list = (List<Student_Fee>) model.get("list");
	...
	private void setBody(Sheet sheet, List<Student_Fee> list) {
		int rowNum = 1;
		for(Student_Fee stdf : list) {
			Row row = sheet.createRow(rowNum++);
			row.createCell(0).setCellValue(stdf.getStudent().getStud_id());
			row.createCell(1).setCellValue(stdf.getStudent().getStud_first_name());
